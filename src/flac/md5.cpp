...
void Md5Context::transform(const uint8_t block[64]) {
...
void Md5Context::update(const uint8_t* data, size_t len) {
...
void Md5Context::finish(uint8_t digest[16]) {
    ...
    for (int i = 0; i < 8; ++i) {
        bits[i] = static_cast<uint8_t>((total_bits >> (i * 8)) & 0xFF);
    }
